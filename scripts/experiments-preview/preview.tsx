...
{path.endsWith("/new")?<ExperimentWizard key={path}/>:path.endsWith("/edit")?<ExperimentWizard key={path} draftId={id}/>:path===`/experiments/${id}`||path===`/experiments/${legacyId}`?<ExperimentDetail key={path} id={path.split("/").at(-1)!}/>:<ExperimentsHome/>}</>;
