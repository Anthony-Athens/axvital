...
export default function Page() {
  ...
  return <div className="mx-auto max-w-4xl px-4 py-6 sm:px-6"><header className="flex flex-col gap-4 border-b pb-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-semibold text-blue-700">Personal briefing</p><h1 className="mt-1 text-3xl font-semibold">Weekly Recap</h1><p className="text-sm text-slate-600">A concise, non-causal summary of your recent structured data.</p></div><button onClick={() => void generate()} disabled={loading || generating} className="min-h-11 rounded-lg bg-blue-600 px-4 font-semibold text-white">{generating ? "Generating…" : recap ? "Regenerate Weekly Recap" : "Generate Weekly Recap"}</button></header>
    ...
    {!loading && !recap ? <section className="mt-6 rounded-xl border border-dashed p-6"><h2 className="text-xl font-semibold">Build your first weekly briefing</h2><p className="mt-2 text-slate-600">A recap becomes more useful after a few days of check-ins, nutrition, activity, or routines.</p><Link href="/today" className="mt-4 inline-flex min-h-11 items-center rounded-lg bg-blue-600 px-4 font-semibold text-white">Track on Today</Link></section> : null}
