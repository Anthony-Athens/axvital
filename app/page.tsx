...
export default function Home() {
  return <>
    ...
    <section id="experiments" className="relative overflow-hidden bg-slate-950 px-4 py-16 text-white sm:px-6 lg:py-24"><div className="absolute right-0 top-0 h-96 w-96 rounded-full bg-blue-600/15 blur-3xl" /><div className="relative mx-auto max-w-7xl">
      ...
      <div className="space-y-3 lg:pt-2">{questions.map((question, index) => <div key={question} className="flex items-center gap-4 rounded-2xl border border-slate-700 bg-slate-900/70 p-5"><span className="text-sm font-semibold text-blue-300">0{index + 1}</span><p className="font-medium leading-7 text-slate-100">{question}</p></div>)}</div></div>
