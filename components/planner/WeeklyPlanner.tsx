...
export function WeeklyPlanner() {
  ...
  async function changeStatus(item: PlannedActivityOccurrence, status: OccurrenceStatus) { setBusyId(item.id); try { const trackable = item.planned_activity?.activity_type === "habit" || Boolean(item.planned_activity?.user_protocol_id); const updated = trackable ? status === "completed" ? await completeHabitOccurrence(supabase, item.id, item.actual_value, item.completion_note) : status === "skipped" ? await skipHabitOccurrence(supabase, item.id) : await reopenHabitOccurrence(supabase, item.id) : await updateOccurrenceStatus(supabase, item.id, status); setOccurrences((values) => values.map((value) => value.id === item.id ? updated : value)); setSuccess(`Activity marked ${status}.`); } catch (value) { logDevError("Failed to change status", value); setError("We couldn’t change the activity status."); } finally { setBusyId(null); } }
  ...
  async function saveProgress(value: number | null, note: string, complete: boolean) { if (!progressItem) return; setBusyId(progressItem.id); try { const updated = await updateHabitProgress(supabase, progressItem.id, value, note || null, complete); setOccurrences((rows) => rows.map((row) => row.id === updated.id ? updated : row)); setProgressItem(null); setSuccess("Habit progress updated."); } catch (reason) { logDevError("Failed to update progress", reason); throw new Error("We couldn’t update your progress."); } finally { setBusyId(null); } }
  ...
  return <div className="mx-auto max-w-6xl px-4 py-6 md:px-6 md:py-10">
    ...
    <div className="mt-3 flex gap-2 overflow-x-auto">{(["all","protocols","habits","other"] as const).map((value) => <button key={value} aria-pressed={filter === value} onClick={() => setFilter(value)} className={`min-h-11 shrink-0 rounded-full px-4 text-sm font-black capitalize ${filter === value ? "bg-violet-600 text-white" : "bg-white"}`}>{value === "all" ? "All activities" : value}</button>)}</div>
    ...
    {!loading ? <div className="mt-5 grid gap-4 lg:grid-cols-2">{days.map((day) => { const items = occurrences.filter((item) => item.scheduled_date === day).filter((item) => filter === "all" || filter === "protocols" ? filter === "all" || Boolean(item.planned_activity?.user_protocol_id) : filter === "habits" ? item.planned_activity?.activity_type === "habit" && !item.planned_activity?.user_protocol_id : !item.planned_activity?.user_protocol_id && item.planned_activity?.activity_type !== "habit"); return <section key={day} className="rounded-3xl border border-slate-200 bg-slate-100/70 p-4"><div className="mb-3 flex items-baseline justify-between"><h2 className="text-xl font-black">{dateLabel(day, { weekday: "long" })}</h2><span className="text-sm font-black text-slate-500">{dateLabel(day, { month: "short", day: "numeric" })}</span></div><div className="space-y-3">{items.map((item) => <OccurrenceCard key={item.id} occurrence={item} busy={busyId === item.id || busyId === item.planned_activity_id} showManagement onStatus={(status) => void changeStatus(item, status)} onEdit={() => { setEditing(item.planned_activity ?? null); setFormOpen(true); }} onDelete={() => item.planned_activity && void remove(item.planned_activity)} onProgress={item.planned_activity?.activity_type === "habit" || item.planned_activity?.user_protocol_id ? () => setProgressItem(item) : undefined} />)}{!items.length ? <p className="rounded-2xl bg-white/70 p-4 text-sm font-semibold text-slate-400">Nothing scheduled.</p> : null}</div></section>; })}</div> : null}
    ...
    {progressItem ? <ProgressModal occurrence={progressItem} saving={busyId === progressItem.id} onClose={() => setProgressItem(null)} onSave={saveProgress} /> : null}
  </div>;
}
