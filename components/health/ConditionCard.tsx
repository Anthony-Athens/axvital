...
export function ConditionCard({ item }: { item: UserCondition }) {
  ...
  return <article className="flex h-full flex-col rounded-xl border border-slate-200 bg-white p-4">
    ...
    <Link href={`/my-health/conditions/${item.id}`} aria-label={`Condition Intelligence for ${name}`} className="mt-auto inline-flex min-h-11 items-center pt-4 text-sm font-semibold text-blue-700 outline-none focus-visible:ring-2 focus-visible:ring-blue-600">Condition Intelligence <span aria-hidden="true" className="ml-1">→</span></Link>
    <Link href={`/health/conditions/${item.id}`} className="inline-flex min-h-11 items-center text-sm text-slate-600">Manage condition</Link>
  </article>;
}
