...
import { days, metrics, PRE_EPISODE_LOOKBACK_DAYS, summarizeLookback, type Activity, type Episode } from "@/lib/condition-intelligence/model";
...
export function ConditionIntelligence({ condition, episodes, healthEvents, now, start, unavailable = [], mode = "app" }: { condition: { id: string; name: string }; episodes: Episode[]; healthEvents: Activity[]; now: number; start: number; unavailable?: string[]; mode?: "app" | "marketing" }) {
  ...
  return <PageContainer className="min-w-0 pb-28 [overflow-wrap:anywhere]">
    ...
    <p className="mb-4 text-sm text-slate-500">Last 18 months · Intervals and time since an episode use start times. Duration uses completed episodes; most recent duration uses the latest end time. Severity uses the latest recorded episode-level value.</p>
    ...
    {!rows.length ? <div className="mt-6"><EmptyState title="Your episode history starts here" description="No episodes started in the last 18 months. Logged episodes in this range will build your condition history."/></div> : null}
    ...
    <section className="mt-8"><h2 className="text-xl font-semibold">What happened before your episodes?</h2><p className="mt-2 text-sm text-slate-500">Events recorded in the {PRE_EPISODE_LOOKBACK_DAYS} days before each episode. Date-only logs cover the seven UTC calendar days before onset day. Counts describe tracking, not causes.</p><p className="mt-2 text-sm text-slate-500">More consistent tracking gives AXVital more context around your episodes.</p><div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{rows.map((episode,index) => <EpisodeLookbackCard key={episode.id} episode={episode} index={index} events={healthEvents} incomplete={Boolean(unavailable.length)}/>)}</div></section>
  </PageContainer>;
}
