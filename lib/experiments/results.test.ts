...
function capture(revision=1,state="ready") {
 ...
 const configuration={model_version:2,name:"Protein 15 & Logged Protein",question:"Does my routine appear associated with logged protein?",analysis_timezone:"UTC",baseline_mode:"historical",baseline_start_date:"2026-08-01",baseline_end_date:"2026-08-07",intervention_start_date:"2026-08-08",intervention_end_date:"2026-08-14",outcomes:[outcome],intervention:{type:"nutrition_target",configuration:{id:B,revision:1,definition:{version:1,domain:"nutrition",kind:"numeric",metric:"protein_grams",operator:"gte",value:15,unit:"g",period:"day"}}}};
...
test("no-capture view offers explicit generation without creating evidence on render",async()=>{const {html}=await render("ready",true);assert.match(html,/No retained analysis/);assert.match(html,/Generate Results/);assert.match(html,/does not guarantee that enough usable evidence exists/);});
for(const [state,label] of [["ready","Logged protein increased"],["insufficient_data","Not enough usable data"],["unable_to_determine","Unable to determine the result"],["blocked_by_integrity","Historical information could not be reconciled"]])test(`results view distinguishes ${label} and separate quality sections`,async()=>{const {html}=await render(state);assert.match(html,new RegExp(label));assert.match(html,/Change tracking/);assert.match(html,/Outcome data/);assert.match(html,/View limitations/);assert.doesNotMatch(html,/evidence_text|sourceId|inputDigest|high confidence|statistically significant/);if(state!=="ready")assert.doesNotMatch(html,/What you tested/);});
test("continuous and ordinal facts, unknown values, history, early-end and pauses render conservatively",async()=>{
 const {data,result,html}=await render();assert.match(html,/Starting-point average/);assert.match(html,/Relative difference:/);assert.match(html,/causal certainty are not established/);assert.match(html,/100%/);assert.match(html,/7 of 7 eligible opportunities followed/);
 ...
 const ordinal=renderToStaticMarkup(createElement(ResultsView,{data,result,selected:1,onSelect(){},onGenerate(){},onOlder(){}}));assert.match(ordinal,/Starting point ranks 2–3/);assert.match(ordinal,/Central observed ranks/);assert.doesNotMatch(ordinal,/Relative difference:/);assert.match(ordinal,/Historical analysis selected/);assert.match(ordinal,/Experiment ended early/);assert.match(ordinal,/not counted as not followed or missing/);assert.match(ordinal,/sm:grid-cols-2/);
 result.eligibility={state:"unsupported_design",reasons:[{code:"EVENT_SURVEILLANCE_DENOMINATOR_UNAVAILABLE",scope:"design",state:"unsupported_design"}]};const unsupported=renderToStaticMarkup(createElement(ResultsView,{data,result,selected:1,onSelect(){},onGenerate(){},onOlder(){}}));assert.match(unsupported,/This result is not supported yet/);assert.match(unsupported,/Missing event logs cannot be treated as confirmed zero/);
});
