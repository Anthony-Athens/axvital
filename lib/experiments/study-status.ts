...
import type { RecurrenceRule } from "../planner/recurrence.ts";
import { collectionHealth, habitExposure, outcomeCompleteness, studyPeriod, unknownCompleteness, unknownExposure, type StudyStatus } from "./study-health.ts";
...
export async function loadStudyStatus(client:SupabaseClient,owner:string,id:string,now=new Date()):Promise<StudyStatus> {
  ...
  if(type==="nutrition_target"||type==="nutrition_pattern")result.intervention.criteria=[`Frozen rule revision: ${source.revision??"unavailable"}`,"Rule evaluation against complete historical intake is not available in this status contract."];
  ...
  if(pauses.error||pauses.data?.length){result.exposure=unknownExposure("Pause history is present or unavailable. Phase-aware eligible opportunities cannot be safely reconstructed.");result.completeness=unknownCompleteness("Pause-adjusted outcome coverage is unavailable.");return result;}
  if(type==="habit"&&source.is_active===true) {
    const live=await client.from("planned_activities").select(habitFields).eq("user_id",owner).eq("id",sourceId).limit(1).abortSignal(AbortSignal.timeout(10000)).maybeSingle();
    if(live.error)result.exposure=unknownExposure("The linked habit could not be read. Retry to check exposure.");
    else if(!live.data||habitFields.split(",").some(key=>JSON.stringify(object(live.data)[key])!==JSON.stringify(source[key==="title"?"name":key]))) {
      result.exposure=unknownExposure("The linked habit differs from its frozen criteria or is unavailable. Existing completions cannot safely be reinterpreted.");
      result.snapshotMessage="The linked habit changed or is unavailable. The original criteria above remain frozen; adherence is unknown.";
    } else {
      const rows=await client.from("planned_activity_occurrences").select("scheduled_date,status").eq("user_id",owner).eq("planned_activity_id",sourceId).gte("scheduled_date",period.start).lte("scheduled_date",period.today<period.end?period.today:period.end).limit(1001).abortSignal(AbortSignal.timeout(10000));
      result.exposure=rows.error||!rows.data||rows.data.length>1000?unknownExposure("Habit evidence was unavailable or truncated. Retry; no missing records are inferred."):habitExposure(source as RecurrenceRule,rows.data,period.start,period.closedEnd,period.today,period.end);
    }
  }
