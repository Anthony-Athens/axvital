...
test("hydrated initial load uses newest revision, exact projection, units and focus without capture",async()=>{
 const t=await setup();try{assert.match(t.doc.body.textContent!,/Logged protein increased/);assert.match(t.doc.body.textContent!,/30 g/);assert.equal(t.doc.activeElement?.id,"analysis-heading");assert.equal(t.transport.calls.filter(c=>c.method==="POST").length,0);assert.equal(t.h.hydrationErrors.length,0);}finally{await t.close();}
});
test("hydrated revision selection clears stale facts, changes history and focuses selected heading",async()=>{
 const t=await setup();try{await t.select(1);assert.match(t.doc.body.textContent!,/Historical analysis selected/);assert.match(t.doc.querySelector("article")!.textContent!,/20 g/);assert.equal(t.dom.window.location.search,"?revision=1");assert.equal(t.doc.activeElement?.textContent,"Logged protein increased");}finally{await t.close();}
});
test("rapid revision switching aborts old reads and late responses cannot replace selected facts",async()=>{
 const t=await setup();try{t.transport.holdReads=true;await t.select(1);assert.equal(t.doc.querySelector("article"),null);await t.select(2);const reads=t.transport.calls.filter(c=>c.url.includes("&revision="));assert.equal(reads.at(-2)?.signal?.aborted,true);await t.h.act(async()=>t.transport.releaseReads());await t.h.settle();assert.match(t.doc.querySelector("article")!.getAttribute("aria-label")!,/revision 2/);assert.equal(t.doc.activeElement?.textContent,"Logged protein increased");}finally{await t.close();}
});
test("no-capture hydration generates exactly once under immediate duplicate clicks",async()=>{
 const t=await setup(true);try{assert.match(t.doc.body.textContent!,/No retained analysis/);const button=[...t.doc.querySelectorAll("button")].find(b=>b.textContent==="Generate Results")!;t.transport.holdMutation=true;await t.h.act(async()=>{button.click();button.click();});assert.equal(t.transport.calls.filter(c=>c.method==="POST").length,1);assert.equal(button.disabled,true);assert.match(button.textContent!,/Capturing evidence/);await t.h.act(async()=>t.transport.releaseMutation?.());await t.h.settle();assert.match(t.doc.querySelector("article")!.getAttribute("aria-label")!,/revision 1/);assert.equal(t.doc.activeElement?.id,"analysis-heading");assert.deepEqual(JSON.parse(JSON.stringify(t.transport.calls.find(c=>c.method==="POST")!.body)),{id:t.transport.metadata.experiment.id,expectedAnalysisRevision:0,expectedLifecycleRevision:1});}finally{await t.close();}
});
...
test("failed selection after displayed success exposes alert without old facts; refresh is read-only",async()=>{
 const t=await setup();try{t.transport.readFailure=1;await t.select(1);assert.equal(t.doc.querySelector("article"),null);assert.match(t.doc.querySelector('[role="alert"]')!.textContent!,/temporarily unavailable/);t.transport.readFailure=null;await t.click("Refresh results");assert.match(t.doc.querySelector("article")!.getAttribute("aria-label")!,/revision 1/);assert.equal(t.transport.calls.filter(c=>c.method==="POST").length,0);}finally{await t.close();}
});
test("a newer indeterminate capture stays selected rather than falling back to older ready facts",async()=>{
 const t=await setup();try{t.transport.next={...t.transport.next,eligibility:{state:"insufficient_data",reasons:[]},facts:null,interpretationTier:"indeterminate"};await t.click("Generate New Analysis");assert.match(t.doc.querySelector("article")!.textContent!,/Not enough usable data/);assert.doesNotMatch(t.doc.querySelector("article")!.textContent!,/What you tested/);await t.select(1);assert.match(t.doc.body.textContent!,/Historical analysis selected/);}finally{await t.close();}
});
