...
test("secondary routes activate More", () => { assert.equal(activeNavigationId("/habits/123"), "more"); assert.equal(activeNavigationId("/protocols/123"), "more"); assert.equal(activeNavigationId("/health/conditions/123"), "more"); assert.equal(activeNavigationId("/profile"), "more"); });
test("My Health appears once in authenticated navigation", () => { assert.equal(moreNavigationItems.filter((item) => item.id === "health").length, 1); assert.equal(moreNavigationItems.find((item) => item.id === "health")?.href, "/health"); });
