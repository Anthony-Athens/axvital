...
import { loadFoodServings, scaleNutrition, searchFoods, selectInitialServing, totalNutrition, type Entry, type Food, type Serving } from "./nutrition.ts";
...
test("serving query failures are not converted to an empty result",async()=>{const query={select:()=>query,eq:()=>query,order:()=>query,then:(resolve:(value:unknown)=>void)=>resolve({data:null,error:{code:"42501",details:"policy",message:"denied"}})};await assert.rejects(()=>loadFoodServings({from:()=>query} as never,"food-id"),/could not be loaded/)});
