...
export async function loadFoodCatalog(client: SupabaseClient): Promise<FoodCatalog> {
  ...
  const [foods, aliases, categories, map, components] = await Promise.all([
    client.from("foods").select("id,name,common_aliases,recipe_unit").eq("is_active", true).limit(1001),
    ...
    client.from("food_components").select("parent_food_id,component_food_id,source,confidence,quantity,unit").limit(1001),
  ]);
  ...
  return { foods: foods.data!.map(food => ({
    id: food.id, name: food.name, recipe_unit: food.recipe_unit,
