...
import { validateFoodResolution, foodPersistence, type FoodResolution } from "../nutrition/food-resolution.ts";
...
const allowed = new Set(["user_id", "event_date", "event_time", "event_type", "input_method", "tags", "food", ...textFields, ...numberFields]);
export function validateHealthEventInput(input: HealthEventInput): HealthEventInput {
  ...
  if (input.food !== undefined) {
    if (!input.food || !["food", "fluid"].includes(input.event_type) || input.food.label !== input.title) return invalid();
    try { validateFoodResolution(input.food); } catch { return invalid(); }
  }
 ...
 * RLS is the authority. One INSERT or invoker RPC makes the entire batch atomic.
...
export async function ingestHealthEvents(client: SupabaseClient, inputs: readonly HealthEventInput[]): Promise<void> {
  ...
  const rows = inputs.map(input => {
    const row = validateHealthEventInput(input);
    return { ...row, ...(row.food ? { food: validateFoodResolution(row.food) } : {}) };
  });
  try {
  ...
  try {
    const { error } = rows.some(row => row.food !== undefined)
      ? await client.rpc("ingest_health_events_with_food", { rows: rows.map(({ food, ...event }) => ({ event, food: food ? foodPersistence(food) : null })) })
      : await client.from("health_events").insert(rows);
