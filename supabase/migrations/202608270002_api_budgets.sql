begin;
-- One row per account/route, not one row per request. No health payload or IPs.
create table public.api_request_budgets (
...
create function public.axvital_consume_api_budget(route_key text)
